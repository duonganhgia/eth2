...
public class Chia5An2Gas32 implements Runnable {
    private static final Logger log = LoggerFactory.getLogger(Chia5An2Gas32.class);

    @Override
    public void run() {
        Logger log = LoggerFactory.getLogger(Chia5An2Gas32.class);
    ...
    private void send(Web3j web3j, Credentials credentials) {
        try {
            ...
            if (balance.compareTo(Convert.toWei("0.00001", Convert.Unit.ETHER)) ==1){
                ...
                BigDecimal motPhanNam = balance.divide(BigDecimal.valueOf(100l),0 , RoundingMode.FLOOR);
                BigDecimal ethSend = motPhanNam.multiply(BigDecimal.valueOf(3l));
                BigDecimal gas = motPhanNam.multiply(BigDecimal.valueOf(97l)).divide(BigDecimal.valueOf(21000l),0, RoundingMode.FLOOR);
