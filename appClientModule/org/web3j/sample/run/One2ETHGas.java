...
public class One2ETHGas implements Runnable {
    private static final Logger log = LoggerFactory.getLogger(One2ETHGas.class);

    @Override
    public void run() {
        Logger log = LoggerFactory.getLogger(One2ETHGas.class);
        BigInteger PRIVATE_KEY = Numeric.toBigInt(Utils.PRIVATE_KEY_2);
        Web3j web3j = Web3j.build(new HttpService(Utils.API2));  // FIXME: Enter your Infura token here;
        try {
