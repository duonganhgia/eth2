...
public class SevenETHGas implements Runnable {
    private static final Logger log = LoggerFactory.getLogger(SevenETHGas.class);

    @Override
    public void run() {
        Logger log = LoggerFactory.getLogger(SevenETHGas.class);
        BigInteger PRIVATE_KEY = Numeric.toBigInt(Utils.PRIVATE_KEY_1);
        Web3j web3j = Web3j.build(new HttpService(Utils.API3));  // FIXME: Enter your Infura token here;
        try {
    ...
    private void send (Web3j web3j, Credentials credentials){
        try {
            ...
            if (balance.compareTo(Convert.toWei("0.0007", Convert.Unit.ETHER)) ==1){
                ...
                TransactionReceipt transferReceipt = transfer.sendFunds(
                        ...
                        BigDecimal.valueOf(0.0007), Convert.Unit.ETHER,Convert.toWei("13", Convert.Unit.GWEI).toBigInteger(),Convert.toWei("21000", Convert.Unit.WEI).toBigInteger())  // 1 wei = 10^-18 Ether
                        .send();
